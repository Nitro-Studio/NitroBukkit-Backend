...
const downloadBuildTools = async () => {
    ...
    await axios({
        ...
        if (time == last) {
            console.log('Latest BuildTools already exists...');
        } else {
    ...
    return new Promise((resolve, reject) => {
        if (typeof stream !== 'undefined' && stream) {
            stream.on('close', () => {
                if (flag == true) {
                    console.log('Finished downloading BuildTools...');
                    resolve();
                } else {
                    reject();
                }
...
const downloadSpigot = async (config) => {
    ...
    return new Promise((resolve, reject) => {
        downloadBuildTools().then(() => {
            ...
            if (fs.existsSync(path.join(jarsDir, `spigot-${version}.jar`))) {
                console.log(`Spigot ${version} already exists...`);
            } else {
                console.log(`Downloading spigot ${version}...`);
                ...
                child.on('exit', (code) => {
                    ...
                    if (flag == true) {
                        console.log(`Finished downloading spigot ${version}...`);
                        resolve();
                    } else {
                        reject();
                    }
...
exports.startSpigot = async (name, version, port, memory, hash, onStart, onConsole, onClose) => {
    newDownload(downloadSpigot, { version: version, memory: memory}, (success) => {
        if (success == true) {
            ...
            onConsole(hash, `Failed to download spigot ${version}`);
        }
    });
}